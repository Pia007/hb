...
// CODE HERE
 const changeHelloBtnBg = () => {
    sayHelloButton.style.backgroundColor = 'black';
    sayHelloButton.style.color = 'white';
}

sayHelloButton.addEventListener('mouseover', changeHelloBtnBg);
...
const changeHelloBtnBgBack = () => {
    sayHelloButton.style.backgroundColor = '#EFEFEF';
    sayHelloButton.style.color = 'black';
}

sayHelloButton.addEventListener('mouseout', changeHelloBtnBgBack);
    ...
    axios.get('http://localhost:3000/animals')
        .then((res) => {
            ...
            for(let i = 0; i < animalsArr.length; i++) {
                ...
                animalPara.textContent = animalsArr[i];
                document.body.appendChild(animalPara);
            }
        })
}
...
// ^ Changing oh My color on mouseover
const changeOhMyBg = () => {
    document.getElementById('animals-button').style.backgroundColor = 'purple';
    document.getElementById('animals-button').style.color = 'white';
}

document.getElementById('animals-button').addEventListener('mouseover', changeOhMyBg);

// ^ Change oh My back to original on mouseout
const changeOhMyBgBack = () => {
    document.getElementById('animals-button').style.backgroundColor = '#EFEFEF';
    document.getElementById('animals-button').style.color = 'black';
}

document.getElementById('animals-button').addEventListener('mouseout', changeOhMyBgBack);
...
const repeatMyParam = () => {
    ...
    const repeatElement = document.getElementById('repeat-button');
...
// ^ Changing oh Reapeat Btn on mouseover
const changeRepeatBg = () => {
    repeatBtn.style.backgroundColor = 'blue';
    repeatBtn.style.color = 'lightblue';

}

repeatBtn.addEventListener('mouseover', changeRepeatBg);

// ^ Change Repeat Btn back to original on mouseout
const changeRepeatBgBack = () => {
    repeatBtn.style.backgroundColor = '#EFEFEF';
    repeatBtn.style.color = 'black';
}

repeatBtn.addEventListener('mouseout', changeRepeatBgBack);
...
const sendQuery = () => {
    axios.get('http://localhost:3000/query-test?query=Lions&query=Tigers&query=Bears')
        .then((res) => {
            console.log(res.data);
    })
}
...
// ^ Changing Query Btn on mouseover
const changeQueryBg = () => {
    queryBtn.style.backgroundColor = 'red';
    queryBtn.style.color = 'pink';

}

queryBtn.addEventListener('mouseover', changeQueryBg);

// ^ Change Query Btn back to original on mouseout
const changeQueryBgBack = () => {
    queryBtn.style.backgroundColor = '#EFEFEF';
    queryBtn.style.color = 'black';
}

queryBtn.addEventListener('mouseout', changeQueryBgBack);
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        CONSOLE : You sent an empty query.

    2: Send more than 1 query on the URL -- what happened? 
        CONSOLE : You sent query: query with value: Lions,Tigers,Bears!
*/
    ...
    axios.post('http://localhost:3000/food', body)
        .then((res) => {
            ...
            for (let i = 0; i < foodArray.length; i++) {
                ...
                let foodPara = document.createElement('li');
                foodPara.innerHTML = foodArray[i];
                document.body.appendChild(foodPara);
            }
    })
...
// ^ Changing add food Btn on mouseover
const changeFoodBtnBg = () => {
    document.querySelector("#add-food-button").style.backgroundColor = 'green';
    document.querySelector("#add-food-button").style.color = 'yellow';

}

document.querySelector("#add-food-button").addEventListener('mouseover', changeFoodBtnBg);

// ^ Change add food Btn back to original on mouseout
const changeFoodBtnBgBack = () => {
    document.querySelector("#add-food-button").style.backgroundColor = '#EFEFEF';
    document.querySelector("#add-food-button").style.color = 'black';
}

document.querySelector("#add-food-button").addEventListener('mouseout', changeFoodBtnBg);